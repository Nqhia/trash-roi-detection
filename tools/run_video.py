...
def main() -> int:
    ...
    wr.writerow(["t_s", "dirty", "alert", "escalated", "n_hot", "n_raw_hot",
                 ...
                 "global_change", "ref_reset", "n_sweep_hot", "n_escalated_hot", "max_run", "verify_failed", "verify_conf",
                 "ms", "hot_cells", "notes"])
    ...
    try:
        for t, frame in iter_frames(args.source, interval):
            ...
            wr.writerow([f"{t:.2f}", int(res.dirty), int(res.alert), int(res.escalated),
                         ...
                         res.n_sweep_hot, res.n_escalated_hot, res.max_run, int(res.verify_failed),
                         f"{res.verify_conf:.3f}",
                         f"{res.ms:.1f}",
